...
describe("Mock contract functions", () => {
  ...
  describe("Save Gravatar from contract call", () => {
    ...
    test("Can save Gravatar from contract", () => {
      ...
      createMockedFunction(
        ...
        .returns([
          ethereum.Value.fromString("Gravatar 48"),
          ethereum.Value.fromString("https://example.com/image48.png"),
        ]);
  ...
  test("Can save transaction from call handler", () => {
    ...
    call.inputValues = [
      new ethereum.EventParam("displayName", ethereum.Value.fromString("name")),
      new ethereum.EventParam(
        "imageUrl",
        ethereum.Value.fromString("example.com")
      ),
    ];
...
describe("Mocked Events", () => {
  ...
  test("Can initialise event with default metadata", () => {
    ...
    assert.addressEquals(
      Address.fromString(DEFAULT_ADDRESS),
      newGravatarEvent.address
    );
    assert.bigIntEquals(
      BigInt.fromI32(DEFAULT_LOG_INDEX),
      newGravatarEvent.logIndex
    );
    assert.bytesEquals(
      Bytes.fromHexString(DEFAULT_BLOCK_HASH) as Bytes,
      newGravatarEvent.block.hash
    );
  });
  ...
  test("Can update event metadata", () => {
    ...
    assert.addressEquals(
      Address.fromString(UPDATED_ADDRESS),
      newGravatarEvent.address
    );
    assert.bigIntEquals(
      BigInt.fromI32(UPDATED_LOG_INDEX),
      newGravatarEvent.logIndex
    );
    assert.bytesEquals(
      Bytes.fromHexString(UPDATED_BLOCK_HASH) as Bytes,
      newGravatarEvent.block.hash
    );
  });
});
...
describe("Entity Store", () => {
  beforeAll(() => {
    ...
    gravatar.owner = Address.fromString(
      "0x90cBa2Bbb19ecc291A12066Fd8329D65FA1f1947"
    );
    ...
    gravatar2.owner = Address.fromString(
      "0x90cBa2Bbb19ecc291A12066Fd8329D65FA1f1947"
    );
  ...
  test("Can use entity.load() to retrieve entity from store", () => {
    ...
    assert.stringEquals(
      "gravatarId0",
      retrievedGravatar!.get("id")!.toString()
    );
  });
  ...
  test("Can update entity that already exists using Entity.save()", () => {
    ...
    gravatar.set(
      "imageUrl",
      Value.fromString("https://i.ytimg.com/vi/MELP46s8Cic/maxresdefault.jpg")
    );
    gravatar.save();
  ...
  test("Can add, get, assert and remove from store", () => {
    assert.fieldEquals(GRAVATAR_ENTITY_TYPE, "23", "id", "23");
  ...
  test("Can assert amount of entities of a certain type in store", () => {
    ...
    while (countEntities(GRAVATAR_ENTITY_TYPE) < 2) {
      ...
      newGravatar.owner = Address.fromString(
        `0x000000000000000000000000000000000000000${counter.toString()}`
      );
